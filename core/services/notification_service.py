...
class NotificationService:
    ...
    def test_email_configuration():
      ...
      # ============================================================================
    # USER ACTION CONFIRMATION EMAILS - CORRECTED VERSION
    # ============================================================================

    @staticmethod
    def send_donation_confirmation_email(item, request=None):
        """Send confirmation email when user successfully donates an item"""
        try:
            subject = f"🎉 Donation Confirmed: {item.title}"
            
            context = NotificationService.get_email_context()
            context.update({
                'item': item,
                'user': item.donor,
                'notification_type': 'donation_confirmation',
                'item_details': {
                    'title': item.title,
                    'category': item.category.name if item.category else 'General',
                    'condition': item.condition.name if item.condition else 'Good',
                    'size': item.size.name if item.size else 'N/A',
                    'listed_date': item.created_at.strftime('%B %d, %Y at %I:%M %p'),
                }
            })
            ...
            if item.donor and item.donor.email:
                success = NotificationService.send_notification(
                    subject=subject,
                    template_path='core/emails/user/donation_confirmation.html',
                    context=context,
                    recipient_list=[item.donor.email],
                    extra_log_info=f"Donation confirmation for: {item.donor.email}"
                )
                
                if success:
                    print(f"✅ Donation confirmation email sent to {item.donor.email}")
                else:
                    print(f"❌ Failed to send donation confirmation to {item.donor.email}")
                
                return success
                
        except Exception as e:
            print(f"❌ Error in send_donation_confirmation_email: {e}")
            return False

    @staticmethod
    def send_request_confirmation_email(clothing_request, request=None):
        """Send confirmation email when user successfully submits a clothing request"""
        try:
            # Use description instead of title for ClothingRequest
            request_description = clothing_request.description[:50] + '...' if clothing_request.description else 'Clothing Request'
            subject = f"🙏 Request Submitted: {request_description}"
            
            context = NotificationService.get_email_context()
            context.update({
                'clothing_request': clothing_request,
                'user': clothing_request.requester,
                'notification_type': 'request_confirmation',
                'request_details': {
                    'description': clothing_request.description,
                    'category': clothing_request.category.name if clothing_request.category else 'General',
                    'urgency': getattr(clothing_request, 'get_urgency_display', lambda: 'Normal')(),
                    'submitted_date': clothing_request.created_at.strftime('%B %d, %Y at %I:%M %p'),
                }
            })
            
            if clothing_request.requester and clothing_request.requester.email:
                success = NotificationService.send_notification(
                    subject=subject,
                    template_path='core/emails/user/request_confirmation.html',
                    context=context,
                    recipient_list=[clothing_request.requester.email],
                    extra_log_info=f"Request confirmation for: {clothing_request.requester.email}"
                )
                
                if success:
                    print(f"✅ Request confirmation email sent to {clothing_request.requester.email}")
                else:
                    print(f"❌ Failed to send request confirmation to {clothing_request.requester.email}")
                
                return success
                
        except Exception as e:
            print(f"❌ Error in send_request_confirmation_email: {e}")
            return False

    @staticmethod
    def send_request_cancellation_confirmation_email(clothing_request, request=None):
        """Send confirmation email when user cancels a clothing request"""
        try:
            # Use description instead of title for ClothingRequest
            request_description = clothing_request.description[:50] + '...' if clothing_request.description else 'Clothing Request'
            subject = f"❌ Request Cancelled: {request_description}"
            
            context = NotificationService.get_email_context()
            context.update({
                'clothing_request': clothing_request,
                'user': clothing_request.requester,
                'notification_type': 'request_cancellation',
                'cancellation_details': {
                    'description': clothing_request.description,
                    'category': clothing_request.category.name if clothing_request.category else 'General',
                    'cancelled_date': timezone.now().strftime('%B %d, %Y at %I:%M %p'),
                }
            })
            ...
            if clothing_request.requester and clothing_request.requester.email:
                success = NotificationService.send_notification(
                    subject=subject,
                    template_path='core/emails/user/request_cancellation.html',
                    context=context,
                    recipient_list=[clothing_request.requester.email],
                    extra_log_info=f"Request cancellation confirmation for: {clothing_request.requester.email}"
                )
                
                if success:
                    print(f"✅ Request cancellation confirmation email sent to {clothing_request.requester.email}")
                else:
                    print(f"❌ Failed to send request cancellation confirmation to {clothing_request.requester.email}")
                
                return success
                
        except Exception as e:
            print(f"❌ Error in send_request_cancellation_confirmation_email: {e}")
            return False

    @staticmethod
    def send_exchange_request_confirmation_email(exchange, request=None):
        """Send confirmation email when user requests an exchange"""
        try:
            subject = f"🔄 Exchange Requested: {exchange.item.title}"
            
            context = NotificationService.get_email_context()
            context.update({
                'exchange': exchange,
                'user': exchange.requester,
                'notification_type': 'exchange_request_confirmation',
                'exchange_details': {
                    'item': exchange.item.title,
                    'donor': exchange.item.donor.username if exchange.item.donor else 'Unknown',
                    'message': exchange.message or 'No additional message provided',
                    'requested_date': exchange.created_at.strftime('%B %d, %Y at %I:%M %p'),
                }
            })
            ...
            if exchange.requester and exchange.requester.email:
                success = NotificationService.send_notification(
                    subject=subject,
                    template_path='core/emails/user/exchange_request_confirmation.html',
                    context=context,
                    recipient_list=[exchange.requester.email],
                    extra_log_info=f"Exchange request confirmation for: {exchange.requester.email}"
                )
                
                if success:
                    print(f"✅ Exchange request confirmation email sent to {exchange.requester.email}")
                else:
                    print(f"❌ Failed to send exchange request confirmation to {exchange.requester.email}")
                
                return success
                
        except Exception as e:
            print(f"❌ Error in send_exchange_request_confirmation_email: {e}")
            return False

    @staticmethod
    def send_exchange_cancellation_confirmation_email(exchange, cancelled_by, request=None):
        """Send confirmation email when user cancels an exchange"""
        try:
            # Determine who cancelled and set appropriate context
            if cancelled_by == exchange.requester:
                user_role = "requester"
                recipient = exchange.requester
                action_by = "You"
            else:
                user_role = "donor"
                recipient = exchange.item.donor
                action_by = "The item owner"
            ...
            context.update({
                'exchange': exchange,
                'user': recipient,
                'notification_type': 'exchange_cancellation',
                'cancellation_details': {
                    'item': exchange.item.title,
                    'status': 'Cancelled',
            ...
            if recipient and recipient.email:
                success = NotificationService.send_notification(
                    subject=subject,
                    template_path='core/emails/user/exchange_cancellation.html',
                    context=context,
                    recipient_list=[recipient.email],
                    extra_log_info=f"Exchange cancellation confirmation for: {recipient.email}"
                )
                
                if success:
                    print(f"✅ Exchange cancellation confirmation email sent to {recipient.email}")
                else:
                    print(f"❌ Failed to send exchange cancellation confirmation to {recipient.email}")
                
                return success
                
        except Exception as e:
            print(f"❌ Error in send_exchange_cancellation_confirmation_email: {e}")
            return False

    @staticmethod
    def send_item_removal_confirmation_email(item, user, removal_type='deleted'):
        """Send confirmation email when user removes their item"""
        try:
            action_text = "Deleted" if removal_type == 'deleted' else "Marked as Taken"
            subject = f"🗑️ Item {action_text}: {item.title}"
            
            context = NotificationService.get_email_context()
            context.update({
                'item': item,
                'user': user,
                'notification_type': 'item_removal_confirmation',
                'removal_details': {
                    'title': item.title,
                    'action': action_text,
                    'removed_date': timezone.now().strftime('%B %d, %Y at %I:%M %p'),
                }
            })
            ...
            if user and user.email:
                success = NotificationService.send_notification(
                    subject=subject,
                    template_path='core/emails/user/item_removal_confirmation.html',
                    context=context,
                    recipient_list=[user.email],
                    extra_log_info=f"Item removal confirmation for: {user.email}"
                )
                
                if success:
                    print(f"✅ Item removal confirmation email sent to {user.email}")
                else:
                    print(f"❌ Failed to send item removal confirmation to {user.email}")
                
                return success
                
        except Exception as e:
            print(f"❌ Error in send_item_removal_confirmation_email: {e}")
            return False

    @staticmethod
    def send_profile_update_confirmation_email(user, changes):
        ...
        try:
            ...
            context.update({
                'user': user,
                'notification_type': 'profile_update_confirmation',
                'update_details': {
                    'changes': changes,
            ...
            if user and user.email:
                success = NotificationService.send_notification(
                    subject=subject,
                    template_path='core/emails/user/profile_update_confirmation.html',
                    context=context,
                    recipient_list=[user.email],
                    extra_log_info=f"Profile update confirmation for: {user.email}"
                )
                
                if success:
                    print(f"✅ Profile update confirmation email sent to {user.email}")
                else:
                    print(f"❌ Failed to send profile update confirmation to {user.email}")
                
                return success
                
        except Exception as e:
            print(f"❌ Error in send_profile_update_confirmation_email: {e}")
            return False
    ...
    def send_password_change_confirmation_email(user):
        ...
        try:
            ...
            context.update({
                'user': user,
                'notification_type': 'password_change_confirmation',
                'change_details': {
                    'changed_date': timezone.now().strftime('%B %d, %Y at %I:%M %p'),
                    'ip_address': 'Unknown',  # You can add IP tracking if needed
                }
            })
            ...
            if user and user.email:
                success = NotificationService.send_notification(
                    subject=subject,
                    template_path='core/emails/user/password_change_confirmation.html',
                    context=context,
                    recipient_list=[user.email],
                    extra_log_info=f"Password change confirmation for: {user.email}"
                )
                
                if success:
                    print(f"✅ Password change confirmation email sent to {user.email}")
                else:
                    print(f"❌ Failed to send password change confirmation to {user.email}")
                
                return success
                
        except Exception as e:
            print(f"❌ Error in send_password_change_confirmation_email: {e}")
            return False

    @staticmethod
    def send_account_deletion_confirmation_email(user_email, username):
        ...
        try:
            ...
            context.update({
                ...
                'deletion_details': {
                    'deleted_date': timezone.now().strftime('%B %d, %Y at %I:%M %p'),
                    'email': user_email,
                }
            })
            ...
            if user_email and '@' in user_email:
                success = NotificationService.send_notification(
                    subject=subject,
                    template_path='core/emails/user/account_deletion_confirmation.html',
                    context=context,
                    recipient_list=[user_email],
                    extra_log_info=f"Account deletion confirmation for: {user_email}"
                )
                
                if success:
                    print(f"✅ Account deletion confirmation email sent to {user_email}")
                else:
                    print(f"❌ Failed to send account deletion confirmation to {user_email}")
                
                return success
                
        except Exception as e:
            print(f"❌ Error in send_account_deletion_confirmation_email: {e}")
            return False