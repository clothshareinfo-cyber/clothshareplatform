    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            try:
                ...
                # ✅ FIXED: Send donation confirmation email to user
                NotificationService.send_donation_confirmation_email(item, request)
                
                # ✅ FIXED: Send notifications - CORRECTED method calls
                NotificationService.notify_new_item_posted(item)  # Only item parameter
                NotificationService.send_new_item_newsletter(item)  # Only item parameter
                
                messages.success(request, 'Your item has been listed successfully! Check your email for confirmation.')
...
def request_clothing(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # ✅ FIXED: Send request confirmation email to user
            NotificationService.send_request_confirmation_email(clothing_request, request)
            
            # ✅ FIXED: Send notifications - CORRECTED method calls
            NotificationService.notify_new_clothing_request(clothing_request)  # Only clothing_request parameter
            NotificationService.send_request_newsletter(clothing_request)  # Only clothing_request parameter
...
def delete_request(request, request_id):
    ...
    if request.method == 'POST':
        ...
        # ✅ ADDED: Send cancellation confirmation email to user
        NotificationService.send_request_cancellation_confirmation_email(clothing_request, request)
        
        messages.success(request, 'Request cancelled successfully! Check your email for confirmation.')
...
def initiate_exchange(request, item_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # ✅ ADDED: Send exchange request confirmation email to user
            NotificationService.send_exchange_request_confirmation_email(exchange, request)
            ...
            NotificationService.notify_exchange_initiated(exchange)
            NotificationService.notify_exchange_request_received(exchange)
            
            messages.success(request, 'Exchange request sent successfully! Check your email for confirmation.')
            return redirect('core:item_detail', item_id=item.id)
        else:
            messages.error(request, 'Please correct the errors below.')
    else:
...
def cancel_exchange(request, item_id):
    ...
    if request.method == 'POST':
        ...
        # ✅ ADDED: Send exchange cancellation confirmation email to user
        NotificationService.send_exchange_cancellation_confirmation_email(exchange, request.user, request)
        ...
        try:
            ...
            NotificationService.notify_exchange_cancelled(exchange, request.user)
        except Exception as e:
            print(f"Notification error: {e}")
        
        messages.success(request, 'Exchange request cancelled successfully. Check your email for confirmation.')
...
def manage_exchange(request, exchange_id):
    ...
    if request.method == 'POST':
        ...
        if action == 'confirm' and exchange.donor == request.user:
            ...
            NotificationService.notify_exchange_confirmed(exchange)
            NotificationService.notify_exchange_request_accepted(exchange)
            ...
            if (exchange.requester == request.user and exchange.status == 'confirmed') or \
               (exchange.donor == request.user and exchange.status == 'confirmed'):
                ...
                NotificationService.notify_exchange_completed(exchange)
            ...
            # ✅ ADDED: Send cancellation confirmation email
            NotificationService.send_exchange_cancellation_confirmation_email(exchange, request.user, request)
            ...
            messages.success(request, 'Exchange cancelled. Check your email for confirmation.')
